...
function vertical_correlation(C,T,tens_a,tens_A,gt::Matrix{Symbol},cxd,cyd,
    ...
    elseif model=="XYZ"
        ...
        if mod(i+j,2) == 0 # pair -> up # 14 24
            ss_xyz = Jx*kron(kron(sx,id),kron(id,sx)) + Jy*kron(kron(sy,id),kron(id,sy)) + Jz*kron(kron(sz,id),kron(id,sz)) + 
        ...
        if mod(i+j,2) == 1 # impair -> down
            ...
            Jx*kron(kron(sx,id),kron(id,sx)) + Jy*kron(kron(sy,id),kron(id,sy)) + Jz*kron(kron(sz,id),kron(id,sz))
        end
    ...
    num = (((((((((Left)*t4c)*aa)*dag(Obs_cax))*t2c)*t4a)*cc)*t2a)*Right)[]
    denom = ((((Left)*t4c*a1*t2c)*t4a*a3*t2a)*Right)[]
